...
/**********************************************************************************************************************************************************************/
/*   gcc test_fonction.c ../src/player.c ../src/ordinateur.c ../src/character.c ../src/tab_character.c ../src/save.c ../src/building.c -o test_fonction -lSDL2 -lm -g */
/**********************************************************************************************************************************************************************/

int main(){
    int test_jeux=0,test_fct_player=1,test_fct_ordi=1; 
    ...
    if(test_fct_player){
        ...
        player_t * main_player = initplayer(OWNER_1);
        player_t * main_player2 = initplayer(OWNER_2);
        ...
        if ( buy_character(&main_player,tab_character,indice_unite) )
        {
        ...
        buy_character(&main_player2,tab_character,indice_unite);
        buy_character(&main_player2,tab_character,++indice_unite);

        main_player->characters->tab[0]->pv = -100;
        ...
        if ( buy_character(&main_player2,tab_character,indice_unite) )
        {
            printf("Achat reussi !\n");
        }
    ...
    if(test_fct_ordi){
        ...
        player_t * play = initplayer(OWNER_1);
        
        
        afficher_ordi(ordin);
        buy_character(&play,tab_character,15);
        sleep(play->delai);
        envoie_char(&play);
    ...
    printf("<------------------------- TEST JEU SANS GRAPHISME ------------------------->\n");
        ordi_t * or = init_ordi();
        player_t * player = initplayer(OWNER_1);
        ...
        while (!quit) {
            while (SDL_PollEvent(&event) != 0) {
                if(event.type==SDL_KEYDOWN){
                    ...
                    case SDLK_KP_1:
                        buy_character(&player,tab_character,12);
                        break;
